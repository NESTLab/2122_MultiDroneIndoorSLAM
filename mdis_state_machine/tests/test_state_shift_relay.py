...
class TestPing(unittest.TestCase):
  ...
  def test_init_state(self: unittest.TestCase) -> None:
    self.assertEqual(verifyInitState(3), True, "Initial state check for relay failed")
  
  # def test_triggered_states(self: unittest.TestCase) -> None:
    self.assertEqual(verifyConnStateChange(3, 4), True, "State shift check for relay state 3-4 failed")
  
  # def test_timed_states(self: unittest.TestCase) -> None:
    self.assertEqual(verifyTimedStateChange(4, 5), True, "State shift check for relay state 4-5 failed")
    self.assertEqual(verifyTimedStateChange(5, 6), True, "Second state shift check for relay state 5-6 failed")
    self.assertEqual(verifyTimedStateChange(6, 3), True, "Second state shift check for relay state 6-3 failed")

  # def test_second_loop(self: unittest.TestCase) -> None:
    self.assertEqual(verifyConnStateChange(3, 4), True, "Second state shift check for relay state 3-4 failed")
    self.assertEqual(verifyTimedStateChange(4, 5), True, "State shift check for relay state 4-5 failed")
    self.assertEqual(verifyTimedStateChange(5, 6), True, "Second state shift check for relay state 5-6 failed")
    self.assertEqual(verifyTimedStateChange(6, 3), True, "Second state shift check for relay state 6-3 failed")
