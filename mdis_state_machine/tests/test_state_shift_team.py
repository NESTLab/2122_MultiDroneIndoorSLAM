...
EXPLORER_LAUNCH_FILE = os.path.join(LAUNCH_DIR, 'test_state_machine.launch')
...
class TestStateShiftExplorer(unittest.TestCase):
	...
	def test_success_state(self: unittest.TestCase) -> None:
		...
		self.assertEqual(verifyConnStateChange(GO_TO_MEET, TRANSIT_TO_MEET, RELAY_NAME, EXPLORER_NAME), True, "State shift check for explorer state 3-4 failed")
    ...
    ## Intermidiate state changes should happen automatically
		...
		self.assertEqual(verifyTransitStateChange(TRANSIT_TO_MEET, MERGE_MAP, RELAY_NAME, DATA_CENTER), True, "State shift check for relay state 4-5 (2) failed")
		self.assertEqual(verifyStateChange(MERGE_MAP, END_MEETING, RELAY_NAME), True, "State shift check for relay state 5-8 failed")
		self.assertEqual(verifyTransitStateChange(END_MEETING, GO_TO_MEET, RELAY_NAME, DATA_CENTER), True, "State shift check for relay state 8-3 failed")
